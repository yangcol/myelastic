...
public class ESSourceMapping {
     ...
     * @return
     */
    public static <T>List<T> getAggregationObjects(JestResult jr, String aggrename, Class<T> type) {
        System.out.println("Enter aagregation objects" + jr.getJsonString() + aggrename);
        JsonArray ja = jr.getJsonObject().get("aggregations").getAsJsonObject().get(aggrename)
                .getAsJsonObject().get("buckets").getAsJsonArray();
        if (null == ja) {
            return null;
        } else if (ja.isJsonNull()){
            return null;
        }
        return ESAbstractAggregationItem.getSourceListAsObjects(ja, type);
    }
